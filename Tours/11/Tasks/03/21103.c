...
double getInformationCapacity(int * hist, int fileSize) {
    double infCap = 0;
    for (int i = 0; i < 256; i++)
        if (hist[i])
...
double getRedundancy(double infCap, double averageLenCodeWord) {
...
int main() {
    ...
    double informationCapacity = getInformationCapacity(hist, fileSize);
    double redundancy = getRedundancy(informationCapacity, (double)codeLenght / fileSize);
